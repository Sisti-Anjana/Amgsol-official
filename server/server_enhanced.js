...
// Function to create HTML table for contact form submissions (from original server)
...
// Contact form route (from original server)
app.post('/api/send-email', async (req, res) => {
  try {
    console.log('📧 Contact Form Submission:', req.body);
    
    const htmlContent = createContactFormTable(req.body);
    
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: process.env.EMAIL_USER,
      cc: 'info@amgsol.com',
      subject: `💬 New Quote Request from ${req.body.name} - ${req.body.organisation}`,
      html: htmlContent
    };
